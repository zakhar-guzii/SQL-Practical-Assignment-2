...
FROM users u
         ...
         LEFT JOIN (SELECT post_id,
                    ...
                    FROM comments
                    GROUP BY post_id) sub ON sub.post_id = p.post_id
         LEFT JOIN (SELECT c1.post_id, c1.comments_text AS recent_comment_text, c1.created_at AS recent_comment_date
                    FROM comments c1
                    WHERE c1.created_at = (SELECT MAX(c2.created_at)
                                           FROM comments c2
                                           WHERE c2.post_id = c1.post_id)) r ON r.post_id = p.post_id
